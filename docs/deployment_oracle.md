...
2. **Runtime configuration on the Compute instance**
   ...
   - Add a `.env.production` file alongside it, derived from [`bot/.env.example`](../bot/.env.example). This file is not committed to
...
- Ensure the VM retains `.env.production` across restarts; the workflow does not provision or overwrite it.
- To rotate credentials, update `.env.production` on the VM and rerun the workflow. Secrets used by the workflow can be rotated in the
